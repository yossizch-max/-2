...
export function DocumentsTab({matterId}:{matterId:string}) {
  ...
  return <section className="workspace-card">
    ...
    <div className="table">
      ...
      {documents?.map(d=><button className="tr" key={d.id} onClick={()=>d.extractionState!=="complete" && extract(d.id)} disabled={busy===d.id}>
        ...
        <span>{busy===d.id?"מחלץ...":d.extractionState}</span>
      </button>)}
    </div>
  </section>;
}
