...
CREATE TABLE matter_folder_bindings(
...
CREATE TABLE verified_fact_sources(
...
CREATE TABLE damage_calculations(
 id TEXT PRIMARY KEY, matter_id TEXT NOT NULL REFERENCES matters(id) ON DELETE CASCADE,
...
CREATE TABLE legal_authority_passages(
...
CREATE TABLE legal_document_sources(
 id TEXT PRIMARY KEY, matter_id TEXT NOT NULL, legal_document_version_id TEXT NOT NULL,
...
CREATE TABLE legal_export_audit(
...
CREATE INDEX idx_bindings_path ON matter_folder_bindings(path_key);
CREATE INDEX idx_docs_matter ON documents(matter_id,category);
CREATE INDEX idx_versions_doc ON document_versions(document_id,created_at);
CREATE INDEX idx_occ_matter ON file_occurrences(matter_id,exists_now);
CREATE INDEX idx_occ_version ON file_occurrences(document_version_id);
CREATE INDEX idx_pages_version ON document_pages(document_version_id,page_number);
CREATE INDEX idx_tasks_matter_due ON tasks(matter_id,status,due_at);
CREATE INDEX idx_deadlines_due ON legal_deadlines(state,due_at);
CREATE INDEX idx_events_start ON calendar_events(matter_id,starts_at);
CREATE INDEX idx_waiting_followup ON waiting_for(status,follow_up_at);
CREATE INDEX idx_ai_runs_matter ON ai_runs(matter_id,started_at);
CREATE INDEX idx_ai_proposals_status ON ai_proposals(matter_id,status);
CREATE INDEX idx_facts_matter ON verified_facts(matter_id,status,stale);
CREATE INDEX idx_fact_sources_fact ON verified_fact_sources(verified_fact_id);
CREATE INDEX idx_damage_matter ON damage_calculations(matter_id,status);
CREATE INDEX idx_authorities_matter ON legal_authorities(matter_id,status);
CREATE INDEX idx_legal_docs_matter ON legal_documents(matter_id,status);
CREATE INDEX idx_legal_versions_doc ON legal_document_versions(legal_document_id,version_number);
CREATE INDEX idx_exports_doc ON legal_export_audit(legal_document_version_id,exported_at);
CREATE INDEX idx_domain_events_matter ON domain_events(matter_id,created_at);

CREATE TRIGGER trg_locked_calc_no_update BEFORE UPDATE ON damage_calculations
WHEN OLD.status='locked' BEGIN SELECT RAISE(ABORT,'LOCKED_DAMAGE_CALCULATION'); END;
CREATE TRIGGER trg_locked_calc_no_delete BEFORE DELETE ON damage_calculations
WHEN OLD.status='locked' BEGIN SELECT RAISE(ABORT,'LOCKED_DAMAGE_CALCULATION'); END;
CREATE TRIGGER trg_locked_input_no_insert BEFORE INSERT ON damage_inputs
...
CREATE TRIGGER trg_locked_input_no_update BEFORE UPDATE ON damage_inputs
WHEN EXISTS(SELECT 1 FROM damage_calculations c WHERE c.id=OLD.calculation_id AND c.status='locked')
BEGIN SELECT RAISE(ABORT,'LOCKED_DAMAGE_INPUTS'); END;
CREATE TRIGGER trg_locked_input_no_delete BEFORE DELETE ON damage_inputs
WHEN EXISTS(SELECT 1 FROM damage_calculations c WHERE c.id=OLD.calculation_id AND c.status='locked')
BEGIN SELECT RAISE(ABORT,'LOCKED_DAMAGE_INPUTS'); END;

CREATE TRIGGER trg_verified_authority_no_update BEFORE UPDATE ON legal_authorities
WHEN OLD.status='verified' BEGIN SELECT RAISE(ABORT,'VERIFIED_AUTHORITY_IMMUTABLE'); END;
CREATE TRIGGER trg_verified_authority_no_delete BEFORE DELETE ON legal_authorities
WHEN OLD.status='verified' BEGIN SELECT RAISE(ABORT,'VERIFIED_AUTHORITY_IMMUTABLE'); END;

CREATE TRIGGER trg_approved_legal_version_no_update BEFORE UPDATE ON legal_document_versions
WHEN OLD.status='approved' BEGIN SELECT RAISE(ABORT,'APPROVED_LEGAL_VERSION_IMMUTABLE'); END;
CREATE TRIGGER trg_approved_legal_version_no_delete BEFORE DELETE ON legal_document_versions
WHEN OLD.status='approved' BEGIN SELECT RAISE(ABORT,'APPROVED_LEGAL_VERSION_IMMUTABLE'); END;
CREATE TRIGGER trg_approved_legal_sources_no_insert BEFORE INSERT ON legal_document_sources
...
CREATE TRIGGER trg_approved_legal_sources_no_update BEFORE UPDATE ON legal_document_sources
WHEN EXISTS(SELECT 1 FROM legal_document_versions v WHERE v.id=OLD.legal_document_version_id AND v.status='approved')
BEGIN SELECT RAISE(ABORT,'APPROVED_LEGAL_SOURCES_IMMUTABLE'); END;
CREATE TRIGGER trg_approved_legal_sources_no_delete BEFORE DELETE ON legal_document_sources
WHEN EXISTS(SELECT 1 FROM legal_document_versions v WHERE v.id=OLD.legal_document_version_id AND v.status='approved')
BEGIN SELECT RAISE(ABORT,'APPROVED_LEGAL_SOURCES_IMMUTABLE'); END;

CREATE TRIGGER trg_export_audit_no_update BEFORE UPDATE ON legal_export_audit BEGIN SELECT RAISE(ABORT,'EXPORT_AUDIT_IMMUTABLE'); END;
CREATE TRIGGER trg_export_audit_no_delete BEFORE DELETE ON legal_export_audit BEGIN SELECT RAISE(ABORT,'EXPORT_AUDIT_IMMUTABLE'); END;
CREATE TRIGGER trg_domain_event_no_update BEFORE UPDATE ON domain_events BEGIN SELECT RAISE(ABORT,'DOMAIN_EVENT_IMMUTABLE'); END;
CREATE TRIGGER trg_domain_event_no_delete BEFORE DELETE ON domain_events BEGIN SELECT RAISE(ABORT,'DOMAIN_EVENT_IMMUTABLE'); END;

CREATE TRIGGER trg_legal_source_matter_guard BEFORE INSERT ON legal_document_sources
...
CREATE TRIGGER trg_legal_damage_matter_guard BEFORE INSERT ON legal_document_versions
